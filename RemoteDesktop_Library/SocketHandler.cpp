...
RemoteDesktop::Network_Return RemoteDesktop::SocketHandler::Exchange_Keys(int dst_id, int src_id, std::wstring aeskey){
	...
	auto ret = SendLoop(this, (char*)&tmp, sizeof(tmp));//id is always sent at the beginning of a connection. This is to accommodate proxy sever
	if (ret == FAILED) return _Disconnect();
	ret = SendLoop(this, _SendBuffer.data(), _SendBuffer.size());
...
RemoteDesktop::Network_Return RemoteDesktop::SocketHandler::_Encrypt_And_Send(NetworkMessages m, const NetworkMsg& msg){
	...
	if (SendLoop(this, _SendBuffer.data(), enph->PayloadLen + sizeof(enph->PayloadLen)) == RemoteDesktop::Network_Return::FAILED) return _Disconnect();
